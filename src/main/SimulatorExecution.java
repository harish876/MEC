...
public class SimulatorExecution {
	...
	public static void main(String args[]) throws IOException {
		...
		//appList.add(new Application("Application1", taskGenerationRate, taskDataEntrySize, taskResultSize, 
		...
		//taskDataEntrySize = (long) (36 * 8 * Math.pow(10, 6));
		...
		appList.add(new Application("Application2", taskGenerationRate, taskDataEntrySize, taskResultSize, computacionalLoadCPUcycles, percentageOfCriticalTasks, deadlineCriticalTasks));
		
		// Lists for number of tasks, IoT devices and MEC servers
		List<Integer> listNumberOfTasks = Arrays.asList(500);
		List<Integer> listNumberIoTDevices = Arrays.asList(100);
		List<Integer> listNumberMECServers = Arrays.asList(0,1,2);
		...
		for(Application app : appList) {
			...
			while(Boolean.TRUE) {
				...
				for(int i = 0; i < numberIoTDevices; i++) {
					if(((systemTime - listOfIoTDevices[i].getBaseTime()) % app.getRateGeneration()) == 0) {
						...
						// Verify is there is an available processing core in the MEC servers
				...
				if(numberTasksCanceledAndConcluded == numberTasks) {
					if(Boolean.FALSE) {
						for(int j = 0; j < numberTasks; j++) {
							System.out.println(listFinishedTasks[j].getIdTask() + "; Energy: " + listFinishedTasks[j].getTotalConsumedEnergy());
						}
					}
			...
			// ---------------------------------------------------------------------------
			if(Boolean.TRUE) {
				String filename = "Excuted Task-" + numberTasks + "-" + numberIoTDevices + "-" + numberMECServers + "-" + (long) app.getComputationalLoad();
	...
	public static void printSimulationLog(String filename, Task[] tasksFinalized, 
		...
		for(int i = 0; i < tasksFinalized.length; i++) {
			...
			if(tasksFinalized[i].getTaskStatus() == TASK_CONCLUDED)
				statusFinal = "TASK_CONCLUDED";
			else
				statusFinal = "TASK_CANCELED";
